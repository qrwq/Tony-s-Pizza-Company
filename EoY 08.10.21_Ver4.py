...
def Order():
    ...
    totalcost=(item1*regularPrice)+(item2*regularPrice)+(item3*regularPrice)+(item4*regularPrice)+(item5*regularPrice)+(item6*regularPrice)+(item7*regularPrice)+(item8*gourmetPrice)+(item9*gourmetPrice)+(item10*gourmetPrice)+(item11*gourmetPrice)+(item12*gourmetPrice)
    #totalcost_Delivery = float(totalcost_Pickup) + int(deliveryCharge)

    #Displays total cost
    if shipmentmethodOption == "Pickup":
        totalpriceAll_outputLabel = Label(new_window, text="$" + str('{:.2f}'.format(totalcost)), font="arial 12", bg="white").place(x=490, y=512)
    else:
        pass
    #else:
